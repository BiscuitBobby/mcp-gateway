...
def get_authorization_server(resource_url, authorization_header):
    ...
    response = requests.get(
        well_known_url, headers={"Authorization": authorization_header}
    )
...
def encode_token_endpoint(token_endpoint):
    ...
    encoded = (
        base64.urlsafe_b64encode(token_endpoint.encode("utf-8"))
        .decode("utf-8")
        .rstrip("=")
    )
    return encoded
...
def register_client(registration_endpoint, redirect_uris, client_name):
    ...
    registration_data = {"redirect_uris": redirect_uris, "client_name": client_name}
...
def complete_oauth_registration(
    resource_url, authorization_header, base_redirect_uri, client_name
):
    ...
    client_info = register_client(
        metadata["registration_endpoint"], [redirect_uri_with_token], client_name
    )
...
def build_authorization_url(
    client_id, authorization_endpoint, redirect_uri, code_challenge
):
...
def handle_callback(
    callback_url, code_verifier, client_id, client_secret, redirect_uri
):
    """
...
def run_oauth_flow(resource_url, auth_header, base_redirect_uri, client_name):
    ...
    client_info = complete_oauth_registration(
        resource_url, auth_header, base_redirect_uri, client_name
    )
