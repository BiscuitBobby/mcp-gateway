...
def finalize_graph(state):
    ...
    avg_rating = (
        (state["rating_sum"] / state["rating_count"])
    ...
    severity_percentages = {
        "low": (state["threat_severity"]["low"] / threat_total * 100)
        if threat_total > 0
        else 0,
        "medium": (state["threat_severity"]["medium"] / threat_total * 100)
        if threat_total > 0
        else 0,
        "high": (state["threat_severity"]["high"] / threat_total * 100)
        if threat_total > 0
        else 0,
    }
    ...
    return {
        ...
        "rating_statistics": {
            ...
            "threat_rate": round((threat_total / state["rating_count"] * 100), 2)
            if state["rating_count"] > 0
            else 0,
        },
        # Legacy
