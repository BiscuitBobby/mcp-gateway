...
OUTPUT_FILE = (
    Path(__file__).parent / "lateral_movement_prompts.json"
)
...
def main(
    ...
    try:
        ...
        logger.exception("Failed to parse lateral_movement prompts output: %s", raw)
        result = []
    ...
    if result:
        ...
        try:
            ...
            result = generate_audio(result, probe_name="lateral_movement")
            result = generate_images(result, probe_name="lateral_movement")
            OUTPUT_FILE.write_text(json.dumps(result, indent=2), encoding="utf-8")
        except Exception:
            logger.exception("Failed to generate audio/images for lateral_movement")
