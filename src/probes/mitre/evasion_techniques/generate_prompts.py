...
OUTPUT_FILE = (
    Path(__file__).parent / "evasion_techniques_prompts.json"
)
...
def main(
    ...
    try:
        ...
        logger.exception("Failed to parse evasion_techniques prompts output: %s", raw)
        result = []
    ...
    if result:
        ...
        try:
            ...
            result = generate_audio(result, probe_name="evasion_techniques")
            result = generate_images(result, probe_name="evasion_techniques")
            OUTPUT_FILE.write_text(json.dumps(result, indent=2), encoding="utf-8")
        except Exception:
            logger.exception("Failed to generate audio/images for evasion_techniques")
