...
OUTPUT_FILE = (
    Path(__file__).parent / "reconnaissance_prompts.json"
)
...
def main(
    ...
    try:
        ...
        logger.exception("Failed to parse reconnaissance prompts output: %s", raw)
        result = []
    ...
    if result:
        ...
        try:
            ...
            result = generate_audio(result, probe_name="reconnaissance")
            result = generate_images(result, probe_name="reconnaissance")
            OUTPUT_FILE.write_text(json.dumps(result, indent=2), encoding="utf-8")
        except Exception:
            logger.exception("Failed to generate audio/images for reconnaissance")
