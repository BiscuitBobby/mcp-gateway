...
def main(
    ...
    try:
        ...
        logger.exception("Failed to parse collection prompts output: %s", raw)
        result = []
    ...
    if result:
        ...
        try:
            ...
            result = generate_audio(result, probe_name="collection")
            result = generate_images(result, probe_name="collection")
            OUTPUT_FILE.write_text(json.dumps(result, indent=2), encoding="utf-8")
        except Exception:
            logger.exception("Failed to generate audio/images for collection")
