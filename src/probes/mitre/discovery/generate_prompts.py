...
OUTPUT_FILE = (
    Path(__file__).parent / "discovery_prompts.json"
)
...
def main(
    ...
    try:
        ...
        logger.exception("Failed to parse discovery prompts output: %s", raw)
        result = []
    ...
    if result:
        ...
        try:
            ...
            result = generate_audio(result, probe_name="discovery")
            result = generate_images(result, probe_name="discovery")
            OUTPUT_FILE.write_text(json.dumps(result, indent=2), encoding="utf-8")
        except Exception:
            logger.exception("Failed to generate audio/images for discovery")
