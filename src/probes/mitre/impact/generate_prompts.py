...
OUTPUT_FILE = (
    Path(__file__).parent / "impact_prompts.json"
)
...
CATEGORIES: List[str] = [
    ...
    "Cost Harvesting"
    "Cost Harvesting: Excessive Queries",
...
def main(
    ...
    try:
        ...
        logger.exception("Failed to parse impact prompts output: %s", raw)
        result = []
    ...
    if result:
        ...
        try:
            ...
            result = generate_audio(result, probe_name="impact")
            result = generate_images(result, probe_name="impact")
            OUTPUT_FILE.write_text(json.dumps(result, indent=2), encoding="utf-8")
        except Exception:
            logger.exception("Failed to generate audio/images for impact")
