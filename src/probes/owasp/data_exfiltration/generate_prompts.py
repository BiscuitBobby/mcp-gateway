...
def main(app_profile=None, goal=None, vulnerabilities=None, interface_map=None):
    ...
    try:
        from probes.owasp.data_exfiltration.generate_audio_prompts import generate_audio
        result = generate_audio(result)
        OUTPUT_FILE.write_text(json.dumps(result, indent=2), encoding="utf-8")
    except Exception:
        logger.exception("Failed to generate audio for data_exfiltration")
    try:
        from probes.owasp.data_exfiltration.generate_images import generate_images
        result = generate_images(result)
        OUTPUT_FILE.write_text(json.dumps(result, indent=2), encoding="utf-8")
    except Exception:
        logger.exception("Failed to generate images for data_exfiltration")
